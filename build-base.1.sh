...
then
      echo "\$proxy is empty"
	docker build -f Dockerfile.base.1 -t ackdev/secure_proxy_securitas-wall-base-1:2026-06-23-r1 . \
	 2>&1 | tee "$log_dir/docker_build.out"

else
      echo "\$proxy is NOT empty"
	docker build -f Dockerfile.base.1 -t ackdev/secure_proxy_securitas-wall-base-1:2026-06-23-r1 . --build-arg \
